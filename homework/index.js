...

{
  ...
  function fetchRepositoryContributors(url, containerDiv) {
    fetchJSON(url)
      .then(contributors => buildRepositoryContributors(contributors, containerDiv))
      .catch(err =>
  ...
  function main(url) {
    ...
    fetchJSON(url)
      .then(repositories => startUpAndBuildSelectList(repositories, root))
      .catch(err => createAndAppend('h1', root, { text: err.message, class: 'alert-error' }));
  }
