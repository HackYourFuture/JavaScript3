...

{
  ...
  function renderRepositories(repository, container) {
    ...
    createAndAppend('td', tr2, { text: repository.description });
    const tr3 = createAndAppend('tr', tbody);
    createAndAppend('td', tr3, { text: 'Forks:' });
    createAndAppend('td', tr3, { text: repository.forks });
    const tr4 = createAndAppend('tr', tbody);
    createAndAppend('td', tr4, { text: 'Updated:' });
    createAndAppend('td', tr4, { text: repository.updated_at });
  }

  function renderContributors(contributors, container) {
    container.innerHTML = '';
    createAndAppend('p', container, { text: 'Contributions' });
    ...
    contributors.forEach(contributor => {
      ...
      createAndAppend('img', a, { src: contributor.avatar_url, width: 100, height: 100 });
      createAndAppend('div', a, { text: contributor.contributions });
    });
  }
  ...
  function main(url) {
    fetchJSON(url, (err, repositories) => {
      ...
      createAndAppend('label', header, { text: 'HYF Repositories' });
      ...
      if (err) {
        ...
        const leftHand = createAndAppend('div', root, { id: 'left-hand' });
        const rightHand = createAndAppend('div', root, { id: 'right-hand' });
        ...
        select.addEventListener('change', () => {
          ...
          fetchJSON(repository.contributors_url, (_err, contributors) => {
            renderContributors(contributors, rightHand);
          });
        });
        renderRepositories(repositories[0], leftHand);

        fetchJSON(repositories[0].contributors_url, (_err, contributors) => {
          renderContributors(contributors, rightHand);
        });
      }
    });
