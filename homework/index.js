...

{
  ...
  function addRowToTable(parent, rowElements = []) {
  ...
  function buildRepositoryInfo(repository, containerDiv) {
    ...
    addRowToTable(tableBody, [
      ...
      {
        type: 'td',
        text: new Date(repository.updated_at).toLocaleDateString(),
      },
    ]);
  }
  ...
  async function fetchRepositoryContributors(url, containerDiv) {
    try {
