...
Hello Dear User! This APP requires a big amount requests to GitHub API.<br>
I would kindly ask you to login instead.
<h1>To generate your GitHub token:</h1>
<ol>
...
<li>Paste your token in the form below</li>
  ...
  const fetchData = async (user, token) => {
    ...
    const dataFromUrls = await Promise.all(
      urls.map(url => cachedFetch(url, options, 30 * 60).then(response => response.json()))
    );
    ...
    const contributorsAll = await Promise.all(
      contributorsUrls.map(url =>
        cachedFetch(url, options, 60 * 60).then(response => (response.status === 204 ? [] : response.json()))
      )
    );
    ...
    const users = await Promise.all(
      usersUrls.map(url => cachedFetch(url, options, 60 * 60).then(response => response.json()))
    );
    const orgusercontrs = users.map(user =>
      Object.assign(user, contributorsUnique.find(contributor => contributor.login === user.login))
    );
...
<span>Details</span><span>Top Magicians<a href=#>Overview all magicians</a></span>
<div id=details>
...
<div id=activity>
    ...
    statsPublicRepos.innerHTML = `<span class="stats-number">${
      org.public_repos
      }</span><span class="stats-text">White Books</span>`;
    statsPrivateRepos.innerHTML = `<span class="stats-number">${
      org.total_private_repos
      }</span><span class="stats-text">Black Books</span>`;
    ...
    latestActivity.innerHTML = orgevents
      .map(x => {
        ...
        <span class="action-was-made">Pushed ${x.payload.size} commit to <a href="${x.repo.url}">${
            x.repo.name
            }</a></span>
        <span class="message"><span class="message-inner">"${x.payload.commits[0].message}"</span></span>`);

        'PullRequestEvent' === type &&
          (type = `${type}-${x.payload.action}`) &&
          (action = `
        <span class="path">
        <a class="path-repo" href="${x.payload.pull_request.head.repo.html_url}">${
        ...
        <span class="message"><span class="message-inner">"${x.payload.comment.body}"</span></span>`);

        'ForkEvent' === type &&
          (action = `Forked a repo <a href="${x.repo.html_url}">${x.repo.name}</a> to <a href="${
            x.payload.forkee.html_url
            }">${x.payload.forkee.name}</a></span><span></span>`);

        'IssueCommentEvent' === type &&
          (action = `<span class="action-was-made">Wrote a <a href="${
            x.payload.comment.html_url
            }">comment</a></span> in issue <a href="${x.payload.issue.html_url}">${x.payload.issue.title}</a></span>
        <span class="message"><span class="message-inner">"${x.payload.comment.body}"</span></span>`);

        'WatchEvent' === type &&
          (action = `Starred a repo <a href="https://github.com/${x.repo.name}">${
            x.repo.name
            }</a></span><span></span>`);

        'DeleteEvent' === type &&
          (action = `Deleted a ${x.payload.ref_type} "${x.payload.ref}" in <a href="https://github.com/${
            x.repo.name
            }">${x.repo.name}</a></span><span></span>`);

        'CreateEvent' === type &&
          (action = `Created a ${x.payload.ref_type} "${x.payload.ref}" with description: ${
            x.payload.description
            } in <a href="https://github.com/${x.repo.name}">${x.repo.name}</a></span><span></span>`);
      ...
      .map(x => {
        ...
        x.contributors !== undefined &&
          (contributor = x.contributors
            .map(
              x => `
    <span class="repo-contr"><i class="repo-contr-image" style="background-image:url(${
                x.avatar_url
                })"></i><span class="repo-contr-name">${x.login}</span><i class="repo-contr-contribs">${
                x.contributions
