...

{
  ...
  function renderDetails(repositories, select) {
    ...
    async function selectElement(repositories) {
      ...
      try {
        const contributors = await fetchJSON(repositories[select.value].contributors_url);
  ...
  function renderContributors(contributors) {
    ...
    for (let i = 0; i < contributors.length; i++) {
      ...
      createAndAppend("img", li, {
        src: contributors[i].avatar_url, alt: contributorName + "'s img"
      });
